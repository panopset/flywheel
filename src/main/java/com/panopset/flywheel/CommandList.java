...
public class CommandList extends MatchableCommand implements
   ...
   */
  public CommandList(final String source, final String innerPiece,
  ...
  public final void resolve(final StringWriter sw) throws FlywheelException {
    ...
    if (lines == null || lines.isEmpty()) {
      Logop.warn(getTemplate().getTemplateSource().getName());
      return;
    }
    ...
    for (String s : lines) {
      ...
      if (Stringop.isPopulated(tokens)) {
        ...
        while (st.hasMoreTokens()) {
          incr++;
          getTemplate().getFlywheel().put("" + incr,
              processValue(st.nextToken()));
        }
      } else if (Stringop.isPopulated(splits)) {
